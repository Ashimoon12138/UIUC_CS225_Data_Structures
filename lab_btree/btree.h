...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    unsigned n = elements.size();
       if(n == 0) return 0; //if empty
       if(val == elements[0] || val < elements[0]) return 0; // insert at the front & due with the one element situation
<<<<<<< HEAD
       if(val > elements[n-1]) return n; // insert at the end & due with the one element situation
       if(val == elements[n-1]) return (n-1);
=======
       if(val == elements[n-1]) return (n-1); //@return the last DataPair's index
       if(val > elements[n-1]) return n; //@insert at the end of

>>>>>>> 5785cde29344cc4c5e7469bb24a99324389e84ec
       unsigned i1 = 0; //represent the head of current scope
       unsigned i2 = n-1; // represent the tail of current scope
       unsigned idx = (i1 + i2)/2; //represent the middle of current scope

       /* constrain the scope and  break in two cases
       1.val = elements[idx]
       2.i1 = idx; or (i1 = i2-1) */

       while(!(val == elements[idx])) {
         if(i1 == idx) break;
         if(val < elements[idx]) {
           i2 = idx;
           idx = (i1 + i2)/2;
       ...
       if(val == elements[idx])
         return idx;
       else return idx+1; //   QUESTION(Done):why I need to return the after one? IN the protacol
       // return 5;
}
