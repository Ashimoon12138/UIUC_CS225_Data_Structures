...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    // auto first = subroot->elements[0];
    // auto second = subroot->elements[1];

    size_t first_larger_idx = insertion_idx(subroot->elements, key);
   //@ need to check weither first_larger_idx equal to n
   ...
   // }
    ...
    if(first_larger_idx < subroot->elements.size()
       && subroot->elements[first_larger_idx] == key) { //if exist, return value
     return subroot->elements[first_larger_idx].value;
   }
    if(subroot->is_leaf) return V(); //if donot exist && is leaf return V();

    return find(subroot->children[first_larger_idx], key); // if not leaf
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // parent->children[child_idx] = new_left;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
      ...
      if(first_larger_idx < subroot->elements.size()
      && pair == subroot->elements[first_larger_idx]) return; //if exist
      if(subroot->is_leaf) {
        subroot->elements.insert(subroot->elements.begin()+first_larger_idx, pair);
        return;
<<<<<<< HEAD
=======
        //@***need return in base case
        //@ try to practice using gdb with it
>>>>>>> 5785cde29344cc4c5e7469bb24a99324389e84ec
      }

      insert(subroot->children[first_larger_idx], pair);

      // QUESTION: why we donot check the subroot=>recursive!
