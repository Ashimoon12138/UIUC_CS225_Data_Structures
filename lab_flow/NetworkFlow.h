...
public:
   ...
   *
   * @return A vector of the vertices from source to sink with greater than zero flow.
   ...
   *
   * @return A constant reference to the state space graph.
   */
...
private:
  ...
  int maxFlow_;
  // int pathCapacity(const Graph &graph, const std::vector<Vertex> & path) const;
